...
def get_Gauss_Similarity_torch(interaction_matrix,name):
    ...
    delta = 1 / t.sum(t.mean(t.pow(X, 2), 0))
    alpha = t.sum(t.pow(X, 2), 1).unsqueeze(dim=1)

    result = t.exp(t.multiply(-delta, alpha + alpha.t() - 2 * X.mm(X.t())))
    result = t.where(t.isnan(result), t.tensor(0.0).cuda(), result)
    result = t.where(t.isinf(result), t.tensor(0.0).cuda(), result)
    # result[t.isnan(result)] = 0
    # result[t.isinf(result)] = 0
    # result = result - t.diag(t.diag(result))
    return result
